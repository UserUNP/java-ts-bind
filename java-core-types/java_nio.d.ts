...
import { IllegalStateException, Comparable, RuntimeException, IndexOutOfBoundsException, Number, Appendable, CharSequence, Readable, UnsupportedOperationException } from 'java.lang';
import { FileDescriptor } from 'java.io';
import { MemorySegment } from 'java.lang.foreign';
...
export class MappedByteBuffer extends ByteBuffer {
  ...
  */
  slice(index: number, length: number): MappedByteBuffer;
  /**
   * {@inheritDoc}
  */
...
import { UserPrincipalLookupService, FileTime, FileStoreAttributeView, PosixFilePermission, FileAttributeView, UserPrincipal, BasicFileAttributes, FileAttribute } from 'java.nio.file.attribute';
...
export class Files {
  ...
  */
   ...
   * directory. Cycle detection is done by classing the {@link
  ...
  */
   ...
   *
   *  This method is equivalent to: {@link readString(Path, Charset)
...
import { FileAttributeView, BasicFileAttributes, FileAttribute } from 'java.nio.file.attribute';
...
export class FileSystemProvider {
  ...
  */
   ...
   * @since 20
  */
  readAttributesIfExists<A>(path: Path, type: Class<A>, ...options: LinkOption[]): A;
}
...
import { MemorySegment, Arena } from 'java.lang.foreign';
...
export class AsynchronousFileChannel extends AsynchronousChannel {
  ...
  */
   ...
   * of size {@link Long#MAX_VALUE}.  If the `position` is non-negative
...
export class FileLock extends AutoCloseable {
  ...
  */
   ...
   *
   * @return  `true` if this lock and the given lock range overlap
...
export class FileChannel extends AbstractInterruptibleChannel {
  ...
  */
   ...
   *          If `arena` is a confined scoped arena, and this method is called from a
   ...
   * @since   19
  */
  map(mode: MapMode, offset: number, size: number, arena: Arena): MemorySegment;
  /**
   * Acquires a lock on the given region of this channel's file.
   *
   ...
   * Long#MAX_VALUE}.  If the `position` is non-negative and the
   ...
   *
   * @throws  NonReadableChannelException
   *          If `shared` is `true` but this channel was not
   *          opened for reading
   *
   * @throws  NonWritableChannelException
...
import { Comparable, CharSequence, Error, IllegalArgumentException, Exception } from 'java.lang';
...
export class Charset extends Comparable<Charset> {
  ...
  */
   ...
   *
   * @return  A charset object for the named charset, or `fallback`
   ...
   * @since 18
  */
  static forName(charsetName: string, fallback: Charset): Charset;
  /**
 ...
 * @param  The type of the context object associated with the event
 *
 * @since 1.7
...
export class Builder {
  ...
  */
   ...
   * @return  this builder
  */
  set principal(principal: UserPrincipal);
  /**
...
export class PosixFileAttributeView extends BasicFileAttributeView {
  ...
  */
   ...
   *          method denies write access to the file.
  */
  set permissions(permissions: Set<PosixFilePermission>);
  /**
   ...
   *          method denies write access to the file.
  */
  set group(group: GroupPrincipal);
}
