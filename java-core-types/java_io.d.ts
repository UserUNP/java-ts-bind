 ...
 * If the system property `jdk.serialFilter` or the {@link java.security.Security}
 ...
 *
 * Setting the `jdk.serialFilterFactory` with {@link System#setProperty(String, String)
...
export class Config {
   ...
   *
   * @return the static JVM-wide deserialization filter or `null` if not configured
  ...
  */
  static get serialFilter(): ObjectInputFilter;
  /**
   ...
   *       the security property `jdk.serialFilter` fails.
  */
  static set serialFilter(serialFilter: ObjectInputFilter);
  /**
   ...
   * @since 17
  */
  static get serialFilterFactory(): BinaryOperator<ObjectInputFilter>;
  /**
   ...
   * setting the `jdk.serialFilterFactory` property in the {@link java.security.Security}
   ...
   * @since 17
  */
  static set serialFilterFactory(serialFilterFactory: BinaryOperator<ObjectInputFilter>);
  /**
...
export class FilterReader extends Reader {
  ...
  */
   ...
   * {@inheritDoc}
   *
   * @throws     IllegalArgumentException  If `n` is negative and the
 ...
 * handling methods discussed above are ignored for class types.
 *
...
export class ByteArrayInputStream extends InputStream {
  ...
  */
   ...
   * @implSpec
   * The `markSupported` method of `ByteArrayInputStream`
   * always returns `true`.
   * @return true
   * @since   1.1
  */
...
export class PrintStream extends FilterOutputStream {
  ...
  */
  ...
  /**
   * {@return the charset used in this `PrintStream` instance}
   *
   * @since 18
  */
  charset(): Charset;
 ...
 * objects. During deserialization the class's canonical constructor is invoked
 * to construct the class object. Certain serialization-related methods, such
 * as readObject and writeObject, are ignored for serializable classs. See
 * 
 * Java Object Serialization Specification, Section 1.13,
...
export class ObjectInputStream extends InputStream {
  ...
  */
   ...
   * with the {@linkplain #getObjectInputFilter() current filter} and the `filter` parameter.
   * The current filter was set in the
   * {@linkplain #ObjectInputStream() ObjectInputStream constructors} by invoking the
   * {@linkplain Config#getSerialFilterFactory() JVM-wide filter factory} and may be `null`.
   ...
   * If the {@linkplain #getObjectInputFilter() current filter} is `non-null`,
   ...
   * @since 9
  */
  set objectInputFilter(objectInputFilter: ObjectInputFilter);
  /**
   * Reads some number of bytes from the input stream and stores them into
   * the buffer array `b`. The number of bytes actually read is
   * returned as an integer.  This method blocks until input data is
 ...
 * written to the ObjectOutputStream in block-data classs. A block data class
 * is composed of a header and data. The block data header consists of a marker
 * and the number of bytes to follow the header.  Consecutive primitive data
 * writes are merged into one block-data class.  The blocking factor used for
 * a block-data class will be 1024 bytes.  Each block-data class will be
 ...
 *
 * Records are serialized differently than ordinary serializable or externalizable
 * objects, see class serialization.
 *
 * @spec serialization/index.html Java Object Serialization Specification
 * @author      Mike Warres
 * @author      Roger Riggs
 ...
 * The JVM-wide filter factory is a function invoked when each {@link ObjectInputStream} is
 ...
 * 
 * The application specific filter factory set via {@link Config#setSerialFilterFactory(BinaryOperator)}
 ...
 *
 * The filter's {@link #checkInput checkInput(FilterInfo)} method is invoked
...
export class ObjectInputFilter {
   ...
   *
   * @apiNote Each filter implementing `checkInput` should return one of the values of {@link Status}.
   * Returning `null` may result in a {@link NullPointerException} or other unpredictable behavior.
   *
  ...
  */
   ...
   * 
   *     {@link Status#UNDECIDED UNDECIDED}, if the `serialClass` is `null`,
   *     {@link Status#ALLOWED ALLOWED}, if the predicate on the class returns `true`,
   ...
   * @param otherStatus a Status to use if the predicate is `false`
   * @return a filter that returns `ALLOWED` if the predicate
   *          on the class is `true`
   * @since 17
  */
  static allowFilter(predicate: Predicate<Class<any>>, otherStatus: Status): ObjectInputFilter;
  /**
   ...
   * 
   *     {@link Status#UNDECIDED UNDECIDED}, if the `serialClass` is `null`,
   *     {@link Status#REJECTED REJECTED}, if the predicate on the class returns `true`,
   *     Otherwise, return `otherStatus`.
   * 
   * 
   ...
   * @param predicate a predicate to test a non-null Class
   * @param otherStatus a Status to use if the predicate is `false`
   * @return returns a filter that returns `REJECTED` if the predicate
   *          on the class is `true`
   * @since 17
  */
  static rejectFilter(predicate: Predicate<Class<any>>, otherStatus: Status): ObjectInputFilter;
  /**
   ...
   * @param anotherFilter a filter to be merged with the filter, may be `null`
   * @return an {@link ObjectInputFilter} that merges the status of the filter and another filter
   * @since 17
  */
  static merge(filter: ObjectInputFilter, anotherFilter: ObjectInputFilter): ObjectInputFilter;
  /**
   ...
   *          is `null`;
   *     Return `REJECTED` if the class is not an {@linkplain Class#isArray() array};
   ...
   * @since 17
  */
  static rejectUndecidedClass(filter: ObjectInputFilter): ObjectInputFilter;
}
/**
...
export class InvalidClassException extends ObjectStreamException {
  ...
  */
   ...
   * @since 19
  */
  constructor(cname: string, reason: string, cause: Throwable);
  /**
...
export class FileDescriptor {
  ...
  */
   ...
   * by a given transaction were classed on a storage medium.
   *
